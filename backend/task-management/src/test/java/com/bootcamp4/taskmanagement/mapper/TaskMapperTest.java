...
class TaskMapperTest {
  ...
  @Test
  void when_convertToDto_is_called_should_return_dto(){
    when(fModelMapper.map(task, TaskDTO.class)).thenReturn(taskDTO);
    TaskDTO taskDTO1=taskMapper.convertToDto(task);
    assertThat(taskDTO).isEqualToComparingFieldByField(taskDTO1);
  }

  @Test
  void when_convertToEntity_is_called_should_return_entity(){
    when(fModelMapper.map(taskDTO, Task.class)).thenReturn(task);
